...
export const useCases = {
  ...
  reducers: {
    createUseCase(state, payload) {
      const { cases } = state;
      const { title, category, description, ref } = payload;

      const newCase = new UseCase({
        title: title,
        category: category,
    ...
    updateUseCaseCategory(state, payload) {
      const { cases } = state;
    ...
    importData(state, payload) {
      ...
      Object.values(newCases).map(useCase => {
        ...
        Object.values(useCase.requirements).map(req => {
          newRequirements[req.id] = new Requirement({
            id: req.id,
            text: req.text
          });
        });
        ...
        Object.values(useCase.inputs).map(param => {
          newInputs[param.id] = new Parameter({
            id: param.id,
            name: param.name,
            type: param.type,
        ...
        Object.values(useCase.outputs).map(param => {
          newOutputs[param.id] = new Parameter({
            id: param.id,
            name: param.name,
            type: param.type,
            required: param.required
        ...
        newList[useCase.id] = new UseCase({
          id: useCase.id,
          title: useCase.title,
