            ...
            return (
              <div>
                <h2 className="use-case-title">
                  UC{index < 10 ? `0${index}` : index}: {useCase.title}
                </h2>
                <h3 className="use-case-section-heading">Category</h3>
                ...
                {requirements.length > 0 && (
                  <ol className="p-s-all">
                    {requirements.map(req => {
                      return <li style={styles.listItem}>{req}</li>;
                    })}
                  </ol>
                )}
                {requirements.length == 0 && (
                          ...
                          {inputs.map(input => {
                            return (
                              <li className="parameter">
                                {input.name}:
                                {<span className="type">{input.type}</span>}{" "}
                                {input.required ? (
                                  <small style={styles.parameterRequired}>
                                    Required
                                  </small>
                                ) : null}
                              </li>
                ...
                {inputs.length == 0 && outputs.length == 0 && (
                  <p className="empty-section">
                    This use case has no parameters! Make sure to add some!
                  </p>
                )}
              </div>
            );
